<template>
  ...
  let reachBottom: Boolean // 是否到达底部
  ...
  const TIPS_TEXT = {
    pulling: props.pullingText || '下拉加载',
  ...
  const checkPosition = (event: TouchEvent) => {
    ...
    const pageY = el.scrollTop + el.offsetHeight
    reachTop = Math.max(pageY, 0) === offsetHeight
    // 如果页面元素在顶端，触发下拉刷新动作
    if (reachTop) {
      state.duration = 0
      touch.start(event)
  ...
  const onTouchMove = (e: TouchEvent) => {
    ...
    if (isTouchable()) {
      ...
      if (reachTop && deltaY.value <= 0 && touch.isVertical()) {
        e?.preventDefault()
        e?.stopPropagation()
        setStatus(ease(Math.abs(deltaY.value)))
      }
    }
  }
